...
#        print("@@@@@@@@@@@@@@@@@@@@@@@")
        res_tmp=image_processor.post_process_object_detection(out, threshold=0.4, target_sizes=target_sizes)[0]
        res_fin=[]
...
for f in ann_lst:
	...
	sum_time+=t2-t1
	f1=open("test_data/detections_sixteenth/"+f,"w")
	scale=4
	for r in results:
	...
	cv2.imwrite("test_data/img_det_sixteenth/"+f[:-4]+'.jpg',img_tmp)
	f1.close()
	c+=1
