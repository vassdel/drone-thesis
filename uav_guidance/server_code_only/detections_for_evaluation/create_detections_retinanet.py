...
for f in ann_lst:
	...
	sum_time+=t2-t1
	f1=open("test_data/detections_retinanet/"+f,"w")
	scale=1
	...
	cv2.imwrite("test_data/img_det_retinanet/"+f[:-4]+'.jpg',img_tmp1)
	f1.close()
	c+=1
...
for f in ann_lst:
        ...
        f1=open("test_data/detections_retinanet_ninth/"+f,"w")
        scale=3
        result=results[0]["boxes"].detach().cpu().numpy()
        conf=results[0]["scores"].detach().cpu().numpy()
        result_conf=result[conf>=thr]
        for r in result_conf:
                ...
                f1.write(",".join(r0)+"\n")
                img_tmp1=cv2.rectangle(img_tmp1,(int(r00[0])//scale,int(r00[1])//scale),(int(r00[2])//scale,int(r00[3])//scale),(0,255,0),2)
        cv2.imwrite("test_data/img_det_retinanet_ninth/"+f[:-4]+'.jpg',img_tmp1)
        f1.close()
        print("Processed image",c)
