...
#        print("@@@@@@@@@@@@@@@@@@@@@@@")
        res_tmp=image_processor.post_process_object_detection(out, threshold=0.1, target_sizes=target_sizes)[0]
        res_fin=[]
...
for f in ann_lst:
	...
	sum_time+=t2-t1
	f1=open("test_data/detections_rt/"+f,"w")
	scale=1
	for r in results:
	...
	cv2.imwrite("test_data/img_det_rt/"+f[:-4]+'.jpg',img_tmp)
	f1.close()
	c+=1
...
for f in ann_lst:
	...
	f1=open("test_data/detections_rt_ninth/"+f,"w")
	scale=3
	for r in results:
		r00=[r[0][0]*scale,r[0][1]*scale,r[0][0]*scale+r[0][2]*scale,r[0][1]*scale+r[0][3]*scale]
		r0=[str(i) for i in r00]
		f1.write(",".join(r0)+"\n")
		img_tmp=cv2.rectangle(img_tmp,(int(r00[0])//scale,int(r00[1])//scale),(int(r00[2])//scale,int(r00[3])//scale),(0,255,0),2)
	cv2.imwrite("test_data/img_det_rt_ninth/"+f[:-4]+'.jpg',img_tmp)
	f1.close()
print("Average inference time:",sum_time/c,"ms")
print("Average inference time:",sum_time_ninth/c,"ms")
