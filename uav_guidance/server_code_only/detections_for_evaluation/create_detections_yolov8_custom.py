...
for f in ann_lst:
	...
	results=model.predict(img_tmp,conf=0.5)
	t2=time.time_ns()//1000000
	sum_time+=t2-t1
	f1=open("test_data/detections_yolov8_custom/"+f,"w")
	scale=1
	result=results[0]
	...
	cv2.imwrite("test_data/img_det_yolov8_custom/"+f[:-4]+'.jpg',img_tmp)
	f1.close()
	c+=1
...
for f in ann_lst:
	...
	f1=open("test_data/detections_yolov8_custom_ninth/"+f,"w")
	scale=3
	result=results[0]
	for b in result.boxes:
		r=b.xyxy
		...
		f1.write(",".join(r0)+"\n")
		img_tmp=cv2.rectangle(img_tmp,(int(r00[0])//scale,int(r00[1])//scale),(int(r00[2])//scale,int(r00[3])//scale),(0,255,0),2)
	cv2.imwrite("test_data/img_det_yolov8_custom_ninth/"+f[:-4]+'.jpg',img_tmp)
	f1.close()
